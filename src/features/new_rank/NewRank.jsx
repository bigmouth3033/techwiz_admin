...
import { createNewRankRequest } from "./api/newRankApi";
...
export default function NewRank() {
  ...
  const createNewRank = createNewRankRequest();
      ...
      createNewRank.mutate(formData, {
        onSuccess: (response) => {
          ...
          if (response.status == 200) {
            navigate("/update_rank?id=" + response.data.id);
          }
        },
        onError: (error) => {
          console.log(error);
