...
export default function UpdateStoryPopUp({ action, story }) {
  ...
  const onCreateNewStory = () => {
    ...
    images.forEach((image) => typeof image == "object" && formData.append("upload_images", image));
    images.forEach((image) => typeof image != "object" && formData.append("old_images", image));

    updateDesignerStory.mutate(formData, {
      onSuccess: (response) => {
