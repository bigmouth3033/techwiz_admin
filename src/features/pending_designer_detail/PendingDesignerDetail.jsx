...
import { getDesignerDetailRequest } from "./api/pendingDesignerDetailApi";
...
export default function PendingDesignerDetail() {
  const approveDesigner = approveDesignerRequest();
  const denyDesigner = denyDesignerRequest();
  const [avatarError, setAvatarError] = useState();
  let [searchParams, setSearchParams] = useSearchParams();
  const getDesignerDetail = getDesignerDetailRequest(searchParams.get("id"));
  let fileRef = useRef();
  ...
  useEffect(() => {
    if (getDesignerDetail.isSuccess && getDesignerDetail.data.status == 200) {
      const data = getDesignerDetail.data.data;
  ...
  }, [getDesignerDetail.status]);

  if (getDesignerDetail.isSuccess && getDesignerDetail.data.status == 404) {
    return (
      <Container>
        <NotFound>
          <img src={notfound} />
  ...
  const onApproveDesigner = (designerId) => {
    ...
    approveDesigner.mutate(formData, {
      onSuccess: (response) => {
        if (response.status == 200) {
          navigate("/pending_approved_designer");
        }
      },
    });
  };

  const onDenyDesigner = (designerId) => {
    const formData = new FormData();
      ...
      <Container>
        <Form>
          <LeftForm>
            ...
            <ButtonContainer>
              <Button1 onClick={() => onDenyDesigner(getDesignerDetail.data.data.id)}>Deny</Button1>
              <Button1 onClick={() => onApproveDesigner(getDesignerDetail.data.data.id)}>
                Accept as new Designer
              </Button1>
            </ButtonContainer>
          </LeftForm>
          <RightForm>
