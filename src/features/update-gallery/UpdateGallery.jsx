...
import colourOptions from "../add_gallery/data/colorOptions";
...
import { getProductByIdRequest, searchProductRequest } from "../add_gallery/api/addGalleryApi";
...
import { getGalleryByIdRequest } from "./api/updateGalleryApi";
...
export default function AddGallery() {
  ...
  const getGalleryById = getGalleryByIdRequest(searchParams.get("id"));
  const [state, dispatch, ACTIONS] = useUpdateGallery();
  const [search, setSearch] = useState("");
  const [header, setHeader] = useState("general");
  const [imageError, setImageError] = useState(false);
  const [isUpdatable, setIsUpdatable] = useState(false);
  const changeGalleryStatus = changeGalleryStatusRequest();
  ...
  useEffect(() => {
    if (
      getGalleryById.isSuccess &&
      getGalleryById.data.status == 200 &&
      getGalleryById.data.data != null
    ) {
      const data = getGalleryById.data.data;

      dispatch({ type: ACTIONS.CHANGE_GALLERY_NAME, next: data.gallery_name });
      dispatch({
        type: ACTIONS.CHANGE_COLOR_TONE,
        next: colourOptions.find((color) => color.value == data.color_tone),
      });
      dispatch({ type: ACTIONS.CHANGE_CONTENT, next: data.description });
      dispatch({
        type: ACTIONS.CHANGE_IMAGE,
        next: data.imageName.split("; ").filter((image) => image.length != 0),
      });
      dispatch({
        type: ACTIONS.CHANGE_ROOM_TYPE,
        next: roomOptions.find((room) => room.value == data.room_type_id),
      });
      dispatch({
        type: ACTIONS.CHANGE_PRODUCT_LIST,
        next: data.galleryDetails.map((detail) => detail.product_id),
      });

      if (
        (admin.data.data.role == "admin" &&
          getGalleryById.data.data.interior_designer_id == null) ||
        (admin.data.data.role == "designer" &&
          admin.data.data.interiordesigner.id == getGalleryById.data.data.interior_designer_id)
      ) {
        setIsUpdatable(true);
      }
    }
  ...
  if (getGalleryById.isSuccess && getGalleryById.data.data == null) {
    return (
      <Container>
        <NotFound>
          <img src={notfound} />
  ...
  const onUpdateGallery = () => {
    ...
    if (isOk) {
      ...
      formData.append("id", getGalleryById.data.data.id);
      ...
      state.image.forEach(
        (item) => typeof item == "object" && formData.append("uploadImages", item)
      );

      state.image.forEach((item) => typeof item != "object" && formData.append("oldImages", item));
  ...
  const onChangeGalleryStatus = (id) => {
    const formData = new FormData();
    formData.append("id", id);

    changeGalleryStatus.mutate(formData, {
      onSuccess: (response) => {
        if (response.status == 200) {
          getGalleryById.refetch();
        }
              ...
              <>
                ...
                <InputContainer>
                  ...
                  <TextEditor
                    ...
                    setState={(value) => {
                      if (isUpdatable) {
                        dispatch({ type: ACTIONS.CHANGE_CONTENT, next: value });
                      }
                    }}
                  />
                </InputContainer>
          ...
          {(isUpdatable || admin.data.data.role == "admin") && (
            <RightContainer>
              <ButtonContainer>
                {isUpdatable && (
                  <div>
                    <h5>This gallery is updatable</h5>
                    <div>
                      <Button1 onClick={() => onUpdateGallery()}>Update</Button1>
                      <Button1 onClick={() => window.location.reload()}>Cancel</Button1>
                    </div>
                  </div>
                )}
                {admin.data.data.role == "admin" && getGalleryById.data?.data && (
                  <div>
                    <Button1 onClick={() => onChangeGalleryStatus(getGalleryById.data.data.id)}>
                      {getGalleryById.data.data.status ? "Deactive" : "Activate"}
                    </Button1>
                  </div>
                )}
              </ButtonContainer>
